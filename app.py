...
def _normalizar_dia_visita_codigo(valor):
    ...
    if valor_str.isdigit():
        num = int(valor_str)
        return num if 1 <= num <= 6 else None
...
def _aliases_dias(codigos):
    aliases = []
    for codigo in (codigos or []):
        aliases.extend(DIA_VISITA_ALIASES.get(codigo, []))
...
def _obter_dias_permitidos_vendedor(vendedor_id):
    cfg = VendedorConfig.query.filter_by(vendedor_id=vendedor_id).first()
    if not cfg:
        return [1, 2, 3, 4, 5, 6]
    return cfg.get_dias_permitidos()
...
def _status_clientes_por_data(cliente_ids, vendedor_id, data_ref):
    ...
    visitas = VisitaCliente.query.filter(
        VisitaCliente.vendedor_id == vendedor_id,
        VisitaCliente.data_visita == data_ref,
        VisitaCliente.cliente_id.in_(cliente_ids),
    ).all()

    for visita in visitas:
        status_map[visita.cliente_id] = visita.status or 'NAO_VISITADO'

    compras_hoje = CompraCliente.query.filter(
        CompraCliente.vendedor_id == vendedor_id,
        db.func.date(CompraCliente.data_compra) == data_ref,
        CompraCliente.cliente_id.in_(cliente_ids),
    ).all()

    for compra in compras_hoje:
        status_map[compra.cliente_id] = 'VENDA'
...
@app.route("/vendedor/clientes-dia")
@login_required
def vendedor_clientes_dia():
    ...
    vendedor = current_user.vendedor
    dia_hoje = _dia_codigo_hoje()
    dias_permitidos = _obter_dias_permitidos_vendedor(vendedor.id)

    if dia_hoje is None:
        clientes = []
        status_map = {}
        resumo = {"VENDA": 0, "SEM_PEDIDO": 0, "NAO_VISITADO": 0}
        return render_template(
            "clientes/dia.html",
            clientes=clientes,
            status_map=status_map,
            resumo=resumo,
            dia_atual_label="Domingo",
            dia_codigo=0,
    ...
    if dia_hoje in dias_permitidos:
        query = query.filter(Cliente.dia_visita.in_(aliases_hoje))
    else:
    ...
    if busca:
        query = query.filter(
            db.or_(
                ...
                Cliente.codigo_cliente.ilike(f"%{busca}%"),
    ...
    resumo = {
        ...
        "SEM_PEDIDO": sum(1 for cid in cliente_ids if status_map.get(cid) == "SEM_PEDIDO"),
        "NAO_VISITADO": sum(1 for cid in cliente_ids if status_map.get(cid) == "NAO_VISITADO"),
    }
...
@app.route("/vendedores/<int:id>/dias-permitidos", methods=["GET", "POST"])
@login_required
def configurar_dias_permitidos_vendedor(id):
    """Admin configura dias de visita permitidos para um vendedor."""
    if current_user.cargo not in ["admin", "gerente"] and not current_user.is_super_admin:
        flash("Acesso negado. Apenas admin/gerente pode configurar dias permitidos.", "danger")
    ...
    if request.method == "GET":
        dias = config_visita.get_dias_permitidos() if config_visita else [1, 2, 3, 4, 5, 6]
        form.dias_permitidos.data = [str(d) for d in dias]
    ...
    if form.validate_on_submit():
        ...
        config_visita.set_dias_permitidos(form.dias_permitidos.data)
        db.session.commit()
        flash("Dias permitidos atualizados com sucesso.", "success")
        return redirect(url_for("lista_vendedores"))

    return render_template(
        "vendedores/dias_permitidos.html",
        vendedor=vendedor,
        form=form,
    )
...
def registrar_visita_cliente(id):
    ...
    form = VisitaClienteForm()

    if request.method == "GET" and visita:
        form.status.data = visita.status
        if visita.justificativa:
            for motivo in [
                ...
                if visita.justificativa.startswith(motivo):
                    ...
                    restante = visita.justificativa.replace(motivo, "", 1).strip(" -:")
                    form.justificativa.data = restante
                    break
            if not form.motivo_sem_pedido.data:
                form.motivo_sem_pedido.data = "Outro"
                form.justificativa.data = visita.justificativa
        form.latitude.data = visita.latitude
        form.longitude.data = visita.longitude
    ...
    if form.validate_on_submit():
        ...
        if visita.status == "SEM_PEDIDO":
            ...
            if motivo == "Outro":
                visita.justificativa = (form.justificativa.data or "").strip()
            else:
                detalhe = (form.justificativa.data or "").strip()
                visita.justificativa = f"{motivo} - {detalhe}" if detalhe else motivo
            visita.pedido_id = None
        elif visita.status == "VENDA":
            compra_hoje = CompraCliente.query.filter(
                CompraCliente.cliente_id == cliente.id,
                CompraCliente.vendedor_id == current_user.vendedor_id,
                db.func.date(CompraCliente.data_compra) == data_ref,
            ).order_by(CompraCliente.data_compra.desc()).first()
            visita.pedido_id = compra_hoje.id if compra_hoje else None
            visita.justificativa = None
        else:
            visita.justificativa = None
            visita.pedido_id = None
        ...
        return redirect(url_for("vendedor_clientes_dia"))

    return render_template("clientes/registrar_visita.html", form=form, cliente=cliente)
...
def relatorio_visitas_clientes():
    ...
    for visita in visitas:
        ...
        if vendedor.id not in produtividade_map:
            produtividade_map[vendedor.id] = {
        ...
        item = produtividade_map[vendedor.id]
        item["clientes"].add(visita.cliente_id)

        if visita.status in ["VENDA", "SEM_PEDIDO"]:
            item["visitados"] += 1

        if visita.status == "VENDA":
            item["vendas"] += 1
        elif visita.status == "SEM_PEDIDO":
            item["sem_venda"] += 1
        ...
        produtividade.append(
            {
                ...
                "clientes": len(item["clientes"]),
                "visitados": item["visitados"],
                "vendas": item["vendas"],
                "sem_venda": item["sem_venda"],
            }
        )

    produtividade.sort(key=lambda x: (x["vendas"], x["visitados"]), reverse=True)

    justificativas = [
        {
            "cliente_nome": v.cliente.nome if v.cliente else "-",
            "vendedor_nome": v.vendedor.nome if v.vendedor else "-",
            "motivo": v.justificativa or "-",
            "data": v.data_visita,
        }
        for v in visitas
        if v.status == "SEM_PEDIDO"
    ]
    justificativas.sort(key=lambda x: x["data"], reverse=True)
...
def registrar_compra(id):
    ...
    if form.validate_on_submit():
        try:
            ...
            try:
                _upsert_visita_venda(cliente, compra)
            except Exception as e:
                app.logger.error(
                    f"Erro ao sincronizar visita por venda {compra.id}: {e}"
                )
