...
FUNCIONARIO_CARGOS_GERENTE = ["gerente", "gerente_manutencao"]
FUNCIONARIO_CARGOS_SUPERVISOR = ["supervisor", "supervisor_manutencao"]
FUNCIONARIO_CARGOS_QUE_PRECISAM_SUPERVISOR = [
    "vendedor",
    "tecnico",
    "auxiliar",
    "usuario",
...
def criar_funcionario():
    ...
    if request.method == "POST":
        try:
            def _render_form(funcionario_form=None):
                return render_template(
            ...
            def _resolver_vinculos_hierarquia(
                *,
                cargo_val: str,
                admin_id_raw_val: str,
                gerente_id_raw_val: str,
                supervisor_id_raw_val: str,
                gerente_id_atual=None,
                supervisor_id_atual=None,
            ):
                gerente_id_res = gerente_id_atual
                supervisor_id_res = supervisor_id_atual

                # Todo GERENTE (incluindo gerente_manutencao) deve estar vinculado a um ADMIN
                if cargo_val in FUNCIONARIO_CARGOS_GERENTE:
                    if not admins_disponiveis:
                        return None, None, (
                            "⚠️ Nenhum administrador disponível para vincular o gerente. "
                            "Cadastre um administrador primeiro."
                        )

                    if not admin_id_raw_val:
                        return None, None, (
                            "⚠️ Selecione o Administrador responsável por este gerente."
                        )

                    try:
                        admin_id = int(admin_id_raw_val)
                    except ValueError:
                        return None, None, "⚠️ Administrador selecionado inválido."
                    ...
                    gerente_id_res = admin.id

                # Todo SUPERVISOR (incluindo supervisor_manutencao) deve estar vinculado a um GERENTE
                if cargo_val in FUNCIONARIO_CARGOS_SUPERVISOR:
                    if not gerentes_disponiveis:
                        return None, None, (
                            "⚠️ Nenhum gerente disponível para vincular o supervisor. "
                            "Cadastre um gerente primeiro."
                        )

                    if not gerente_id_raw_val:
                        return None, None, (
                            "⚠️ Selecione o Gerente responsável por este supervisor."
                        )

                    try:
                        gerente_id_val = int(gerente_id_raw_val)
                    except ValueError:
                        return None, None, "⚠️ Gerente selecionado inválido."

                    gerente_responsavel = Usuario.query.get(gerente_id_val)
                    if not gerente_responsavel or gerente_responsavel.cargo not in (
                        FUNCIONARIO_CARGOS_GERENTE
                    ):
                        return None, None, (
                            "⚠️ Gerente responsável não encontrado ou inválido."
                        )

                    gerente_id_res = gerente_responsavel.id

                # Todos VENDEDOR, TÉCNICO, AUXILIAR e USUÁRIO devem estar vinculados a um SUPERVISOR
                if cargo_val in FUNCIONARIO_CARGOS_QUE_PRECISAM_SUPERVISOR:
                    if not supervisores_disponiveis:
                        return None, None, (
                            "⚠️ Nenhum supervisor disponível para vincular este funcionário. "
                            "Cadastre um supervisor primeiro."
                        )

                    if not supervisor_id_raw_val:
                        return None, None, (
                            "⚠️ Selecione o Supervisor responsável por este funcionário."
                        )

                    try:
                        supervisor_id_val = int(supervisor_id_raw_val)
                    except ValueError:
                        return None, None, "⚠️ Supervisor selecionado inválido."
                    ...
                    if not supervisor_responsavel or supervisor_responsavel.cargo not in (
                        FUNCIONARIO_CARGOS_SUPERVISOR
                    ):
                        return None, None, (
                            "⚠️ Supervisor responsável não encontrado ou inválido."
                        )

                    supervisor_id_res = supervisor_responsavel.id

                return gerente_id_res, supervisor_id_res, None
            ...
            if cargo not in FUNCIONARIO_CARGOS_PERMITIDOS:
            ...
            gerente_id, supervisor_id, hierarquia_erro = _resolver_vinculos_hierarquia(
                cargo_val=cargo,
                admin_id_raw_val=admin_id_raw,
                gerente_id_raw_val=gerente_id_raw,
                supervisor_id_raw_val=supervisor_id_raw,
                gerente_id_atual=None,
                supervisor_id_atual=None,
            )
            if hierarquia_erro:
                flash(hierarquia_erro, "warning")
                return _render_form(None)
...
def editar_funcionario(id):
    ...
    if request.method == "POST":
        try:
            def _render_form(funcionario_form=funcionario):
                return render_template(
                    "funcionarios/form.html",
                    funcionario=funcionario_form,
                    admins_disponiveis=admins_disponiveis,
            ...
            def _resolver_vinculos_hierarquia(
                *,
                ...
                if cargo_val in FUNCIONARIO_CARGOS_QUE_PRECISAM_SUPERVISOR:
                    ...
                    supervisor_id_res = supervisor_responsavel.id
                elif cargo_val not in ["gerente", "supervisor"]:
                    supervisor_id_res = supervisor_id_res or None
            ...
            if not all([nome, email, cargo]):
                ...
                return _render_form(funcionario)
            ...
            gerente_id, supervisor_id, hierarquia_erro = _resolver_vinculos_hierarquia(
                ...
                gerente_id_raw_val=gerente_id_raw,
                supervisor_id_raw_val=supervisor_id_raw,
                gerente_id_atual=funcionario.gerente_id,
                supervisor_id_atual=funcionario.supervisor_id,
            )
            if hierarquia_erro:
                flash(hierarquia_erro, "warning")
                return _render_form(funcionario)
...
def diagnostico_excel():
    ...
    try:
        import numpy
        diagnostico["numpy_version"] = numpy.__version__
    except Exception as e:
